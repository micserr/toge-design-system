...
export const useCheckbox = (props: CheckboxPropTypes, emit: SetupContext<CheckboxEmitTypes>['emit']) => {
  ...
  const checkboxClasses: ComputedRef<CheckboxClasses> = computed(() => {
    ...
    const inputCheckboxClasses = classNames(
      'spr-h-[15px] spr-w-[15px] spr-appearance-none spr-rounded-[2.5px] spr-border-color-supporting spr-border-[1.25px] spr-border-solid spr-m-1',
      'spr-transition spr-duration-150 spr-ease-in-out',
      {
        'spr-background-color-brand-base spr-border-color-brand-base spr-background-color-brand-hover spr-border-color-brand-hover': modelValue.value || checked.value,
        'spr-background-color-hover': !modelValue.value && !checked.value,
    ...
    const inputCheckboxCheckIconClasses = classNames(
      'spr-h-[10px] spr-w-[10px] spr-flex spr-items-center spr-justify-center spr-pointer-events-none spr-absolute spr-left-1/2 spr-top-1/2 -spr-translate-x-1/2 -spr-translate-y-1/2 spr-transform spr-font-bold spr-opacity-0',
      {
    ...
    const labelClasses = classNames('spr-body-xs-regular spr-text-color-strong spr-block', {
      'spr-text-color-on-fill-disabled': disabled.value,
    });
