  ...
  test.describe('Props - Styling Options', () => {
    ...
    test('renders with custom color styling', async ({ mount }) => {
      ...
      await expect(component).toHaveAttribute(
        'style',
        expect.stringContaining('background-color: rgba(255, 87, 51, 0.125)'),
      );
    });
  ...
  test.describe('Edge Cases', () => {
    ...
    test('handles long text content', async ({ mount }) => {
      ...
      const longDescription =
        'This is an extremely long description that should test how the component handles text wrapping and overflow scenarios';
